...
import { ArrowUpRight, CreditCard, DollarSign, Info, Link2, Plus, RefreshCw, Wallet } from "lucide-react"
...
export default function WalletPanel({ roomId = "", onRequestConnect, onRequestClose }: WalletPanelProps) {
  const { isConnected, isLoading: isWalletLoading, connectedUsername, walletData, disconnect } = useWallet()
  ...
  const loadBalances = async () => {
    ...
    try {
      ...
      const data = await get(`getBalances/${connectedUsername}`) as any
  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 space-y-4">
        ...
        <Button onClick={() => {
          // Dispatch event to open connection dialog in drawer
          if (typeof window !== "undefined") {
            window.dispatchEvent(new CustomEvent("open-wallet-drawer"))
          }
        }}>
          Connect Wallet
        </Button>
